package com.baselibrary.typhoon;
...
//				String url = "http://typhoon.nmc.cn/weatherservice/typhoon/jsons/list_" + dto.yearly + "?t="+new Date().getTime();
				String url = "http://scapi.weather.com.cn/weather/typhoon?year=list_" + dto.yearly + "&test=ncg";
...
//		String url = "http://typhoon.nmc.cn/weatherservice/typhoon/jsons/list_" + yearList.get(0).yearly + "?t="+new Date().getTime();
		String url = "http://scapi.weather.com.cn/weather/typhoon?year=list_" + yearList.get(0).yearly + "&test=ncg";
		if (!TextUtils.isEmpty(url)) {
...
//									String detailUrl = "http://typhoon.nmc.cn/weatherservice/typhoon/jsons/view_" + data.id + "?t="+new Date().getTime();
									String detailUrl = "http://scapi.weather.com.cn/weather/typhoon?view=view_" + data.id + "&test=ncg";
...
//				String detailUrl = "http://typhoon.nmc.cn/weatherservice/typhoon/jsons/view_" + dto.id + "?t="+new Date().getTime();
				String detailUrl = "http://scapi.weather.com.cn/weather/typhoon?view=view_" + dto.id + "&test=ncg";
				asyncQueryDetail(detailUrl, tvTyphoonName.getText().toString());
								...
								if (!itemArray2.get(11).equals(null)) {
									...
									for (int n = 0; n < array11.length(); n++) {
										...
										for (int i = 0; i < itemArray11.length(); i++) {
											...
											long ttt = t1+t2;
											String ttime = sdf.format(new Date(ttt));
...
//						if (isAnimate || null != lastShikuangPoint) {
	...
	private void drawRoute(TyphoonDto start, TyphoonDto end) {
		...
		if (!TextUtils.isEmpty(end.type)) {//实况线
	...
	public boolean onMarkerClick(Marker arg0) {
		...
		if (!TextUtils.isEmpty(snippet[0])) {
		...
		if (!TextUtils.isEmpty(snippet[1])) {
