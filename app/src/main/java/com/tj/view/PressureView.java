...
public class PressureView extends View{
	...
	public void setData(List<FactCheckDto> dataList) {
		if (!dataList.isEmpty()) {
			if (dataList.get(0).pressure >= 99999) {
				min = 800;
			}else {
			...
			for (int i = 0; i < tempList.size(); i++) {
				if (maxValue <= tempList.get(i).pressure && tempList.get(i).pressure < 99999) {
...
//		canvas.drawRect(CommonUtil.dip2px(mContext, 10), CommonUtil.dip2px(mContext, 10), w-CommonUtil.dip2px(mContext, 10), h-bottomMargin, pp);
		...
		if (min <= 800) {
			if (totalDivider <= 300) {
				itemDivider = 50;
			}else if (totalDivider <= 1000 && totalDivider > 300) {
				itemDivider = 100;
			}else if (totalDivider > 1000) {
				itemDivider = 200;
			}
		}else {
			if (totalDivider <= 5) {
		...
		for (int i = 0; i <= totalDivider; i+=itemDivider) {
			...
			canvas.drawText(String.valueOf(i+min), CommonUtil.dip2px(mContext, 5), dividerY-CommonUtil.dip2px(mContext, 2), textP);
		}
